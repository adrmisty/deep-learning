...
class Model(torch.nn.Module):
    ...
    def train_epoch(self, dataset: MNIST.Dataset) -> None:
        ...
        for batch in dataset.batches(self._args.batch_size, shuffle=True):
            ...
            # TODO-solved: Compute the predicted logits of the batch images by calling `self(...)`.
            logits = self(images)

            # TODO-solved: Compute the probabilities of the batch images using `torch.softmax`.
            probabilities = torch.softmax(logits, dim=1)

            # TODO-solved: Manually compute the loss:
            ...
            loss = -torch.log(probabilities[torch.arange(labels.shape[0]), labels]).mean()
            ...
            # TODO-solved: Compute the gradient of the loss with respect to parameters using
            ...
            with torch.no_grad():
                for parameter, grad in zip(parameters, gradients):
                    # TODO-solved: Perform the SGD update with learning rate `self._args.learning_rate`
                    # for the parameter and computed gradient. You can modify the
                    # parameter value directly with `-=`, or you can use `parameter.sub_`.
                    parameter -= self._args.learning_rate * grad
    ...
    def evaluate(self, dataset: MNIST.Dataset) -> float:
        ...
        with torch.no_grad():
            ...
            for batch in dataset.batches(self._args.batch_size):
                # TODO-solved: Compute the logits of the batch images as in the training,
