...
def get_optimizer(args : argparse.Namespace, model : npfl138.TrainableModule) -> torch.optim.Optimizer:
    ...
    # - For `SGD`, if `args.momentum` is specified, use Nesterov momentum.
        case "SGD":
            optimizer = torch.optim.SGD(
                ...
                momentum=args.momentum if args.momentum else 0, nesterov=bool(args.momentum)
            )
        case "Adam":
...
def get_scheduler(args : argparse.Namespace, optimizer : torch.optim.Optimizer, train : torch.utils.data.DataLoader):
    scheduler = None
    ...
    if args.decay and args.learning_rate_final:
        iterations = len(train) * args.epochs
    ...
    #     `end_factor`, and `total_iters` appropriately;
        if args.decay == "linear":
            scheduler = torch.optim.lr_scheduler.LinearLR(
                optimizer, start_factor=1.0, end_factor=args.learning_rate_final / args.learning_rate, total_iters=iterations
            )
    ...
    #     appropriately (be careful to compute it using float64 to avoid precision issues);
            ...
            gamma = float((args.learning_rate_final / args.learning_rate) ** (1 / iterations))
    ...
    #     and `eta_min` appropriately.
            ...
            scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
                optimizer, T_max=iterations, eta_min=args.learning_rate_final
            )
...
def main(args: argparse.Namespace) -> dict[str, float]:
    ...
    # TODO-solved: Use the required `args.optimizer` (either `SGD` or `Adam`) with
    ...
    sch = get_scheduler(args, opt, train)
