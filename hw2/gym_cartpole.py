...
class Model(npfl138.TrainableModule):
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        # TODO-solved: Create the model layers, with the last layer having 2 outputs.
        # To store a list of layers, you can use either `torch.nn.Sequential`
        # or `torch.nn.ModuleList`; you should *not* use a Python list.

        # considering that we are working with a small (100 elem.) and potentially noisy dataset
        # model should not overfit to this data, therefore network should not be too deep
        # Two hidden layers with decreasing neurons (16 → 8) provide a good trade-off between expressiveness and regularization.
        # 2 outputs -> the number of possible actions
        # inputs -> cartpole features (cart position, cart velocity, pole α, pole ω), and not too many neurons (16 would work)
        # hidden layers ->
        #   - reLU for non-linearity
        #   - Linear(32, 16), gradually reduce number of neurons and condensing representations linearly
        #   - reLU for non-linearity
        # output -> Linear(16, 2), 2 possible actions (left, right)

        self.model = torch.nn.Sequential(
            torch.nn.Linear(GymCartpoleDataset.FEATURES, 64),
            torch.nn.Tanh(),
            torch.nn.Dropout(0.2),  # Dropout layer
    ...
    def forward(self, inputs: torch.Tensor) -> torch.Tensor:
        # TODO-solved: Run your model. Because some inputs are on a CPU, you should
...
def main(args: argparse.Namespace) -> torch.nn.Module | None:
    ...
    if not args.evaluate:
        ...
        dataset = GymCartpoleDataset() # TODO-solved: provided 100-element dataset, small and noisy
        train = torch.utils.data.DataLoader(dataset.train, args.batch_size, shuffle=True)

        model = Model(args)

        # TODO-solved: Configure the model for training.
        # improvements: high lr=0.01 --> lr=0.001 --> lr=0.0005
        model.configure(optimizer=torch.optim.RMSprop(model.parameters(), lr=0.0003, momentum=0.9), loss=torch.nn.CrossEntropyLoss())

        # TODO-solved: Train the model. Note that you can pass a list of callbacks to the
