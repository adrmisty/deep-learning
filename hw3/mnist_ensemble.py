...
def main(args: argparse.Namespace) -> tuple[list[float], list[float]]:
    ...
    for i, model in enumerate(models):
        # TODO-solved: Compute the accuracy on the dev set for the individual `models[model]`.
        model_outputs = torch.from_numpy(np.array(model.predict(dev, data_with_labels=True)))
        # logits -> probabilities
        preds.append(torch.nn.functional.softmax(model_outputs, dim=1)) 
        labels = torch.cat([labels for _, labels in dev])

        individual_accuracy = torchmetrics.functional.accuracy(
            model_outputs.argmax(dim=1), labels, task="multiclass", num_classes=MNIST.LABELS
        )

        # TODO-solved: Compute the accuracy on the dev set for the ensemble `models[0:model+1]`.
        #
        ...
        ensemble_preds = torch.stack(preds[:i+1]).mean(dim=0)
        ensemble_accuracy = torchmetrics.functional.accuracy(
            ensemble_preds.argmax(dim=1), labels, task="multiclass", num_classes=MNIST.LABELS
        )
