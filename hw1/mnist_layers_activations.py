...
def set_model(H_layers, H_layer_size, activation_f, ) -> torch.nn.Sequential:
    """
    ...
    model : torch.nn.Sequential
        finished, built constructed neural network model
    """
    layers = [torch.nn.Flatten()]
    # initial input layer size
    input_size = MNIST.H * MNIST.W * MNIST.C
    
    for _ in range(H_layers):
        # per layer, add it to network
        layers.append(torch.nn.Linear(input_size, H_layer_size))
        activation = A[activation_f]
        # include activation function
        if activation:
            layers.append(activation)
        # hidden layer size
        input_size = H_layer_size
...
def main(args: argparse.Namespace) -> dict[str, float]:
    ...
    # TODO-done: Finish the model.
    model = set_model(args.hidden_layers, args.hidden_layer_size, args.activation)
