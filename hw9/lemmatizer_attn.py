...
class WithAttention(torch.nn.Module):
    ...
    def forward(self, inputs, states):
        ...
        projections = self._project_decoder_layer(states)[:, np.newaxis, :] + self._encoded_projected
        ...
        weights = torch.tanh(projections)
        weights = self._output_layer(weights)
        ...
        attention = self._encoded * weights
        ...
        conc = torch.cat([inputs, attention], dim=1)
...
class Model(npfl138.TrainableModule):
    def __init__(self, args: argparse.Namespace, train: MorphoDataset.Dataset) -> None:
        ...
        self._target_rnn_cell = WithAttention(torch.nn.GRUCell(args.cle_dim + args.rnn_dim, args.rnn_dim), args.rnn_dim)
        ...
        if not args.tie_embeddings:
            ...
            self._target_embedding = lambda inputs: torch.nn.functional.embedding(inputs, self._target_output_layer.weight) * (args.rnn_dim ** 0.5)
    ...
    def encoder(self, words: torch.Tensor) -> torch.Tensor:
        ...
        packed = torch.nn.utils.rnn.pack_padded_sequence(hidden, torch.sum(words != MorphoDataset.PAD, dim=-1).cpu(), batch_first=True, enforce_sorted=False)
        fhs, _ = self._source_rnn(packed)
        fhs, _ = torch.nn.utils.rnn.pad_packed_sequence(fhs, batch_first=True)
        combined_outputs = fhs.chunk(2, dim=-1)
        encoded = combined_outputs[0] + combined_outputs[1]
    ...
    def decoder_training(self, encoded: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
        ...
        dec_inputs = torch.nn.functional.pad(targets[:, :-1], (1, 0), value=MorphoDataset.BOW)
        ...
        hidden = self._target_embedding(dec_inputs)
        states = encoded[:, 0]
        outputs = []
        for t in range(hidden.size(1)):
            states = self._target_rnn_cell(hidden[:, t], states)
            outputs.append(states)

        hidden = self._target_output_layer(torch.stack(outputs, dim=1))
    ...
    def decoder_prediction(self, encoded: torch.Tensor, max_length: int) -> torch.Tensor:
        ...
        index = 0
        inputs = torch.full([batch_size], MorphoDataset.BOW, dtype=torch.long, device=device)
        states = encoded[:, 0]
        results = []
        result_lengths = torch.full([batch_size], max_length, dtype=torch.long, device=device)
