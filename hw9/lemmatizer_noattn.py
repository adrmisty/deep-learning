...
class Model(npfl138.TrainableModule):
    ...
    def encoder(self, words: torch.Tensor) -> torch.Tensor:
        ...
        _, lhs = self._source_rnn(packed)
        lhs = torch.sum(lhs, dim = 0)
        return lhs
    ...
    def decoder_training(self, encoded: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
        ...
        packed = torch.nn.utils.rnn.pack_padded_sequence(
            self._target_embedding(dec_inputs), targets_len, batch_first=True, enforce_sorted=False
        )
        hidden, _ = self._target_rnn(packed, encoded[np.newaxis])
        hidden, _ = torch.nn.utils.rnn.pad_packed_sequence(hidden, batch_first=True)
        hidden = self._target_output_layer(hidden).permute(0, 2, 1)
        return hidden
